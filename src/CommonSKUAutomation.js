...
class CommonSKUAutomation {
     ...
     * Returns array: [dayOfWeekFile, dailyFile]
     * Example: ['dash-monday-112025.csv', 'dash-daily-112025.csv']
     */
    getCustomFilenames(reportType) {
        const now = new Date();
        ...
        const reportPrefix = reportType === 'dashboard' ? 'dash' :
            reportType === 'pipeline' ? 'pipe' :
                'sr';
        return {
            dayOfWeek: `${reportPrefix}-${dayOfWeek}-${month}${year}.csv`,
            daily: `${reportPrefix}-daily-${month}${year}.csv`
        };
    }
    async initialize() {
        try {
    ...
    async exportDashboardReport(dateRange = 'Today') {
        ...
        try {
            ...
            const filenames = this.getCustomFilenames('dashboard');
            ...
            // Save both files (day-of-week and daily)
            const files = [filenames.dayOfWeek, filenames.daily];
            ...
            return path.join(targetFolder, filenames.dayOfWeek);
        }
        catch (error) {
    ...
    async exportPipelineReport(dateRange = 'Today') {
        ...
        try {
            ...
            await download.saveAs(tempFilePath);
            // Get filenames and folder
            const filenames = this.getCustomFilenames('pipeline');
            const targetFolder = this.getFolderForDateRange(dateRange);
            // Create folder if it doesn't exist
            if (!fs.existsSync(targetFolder)) {
                fs.mkdirSync(targetFolder, { recursive: true });
    ...
    async exportSalesOrdersReport(dateRange = 'This Week') {
        ...
        try {
            ...
            await this.page.waitForTimeout(5000);
            // Apply filters - Sales Rep doesn't need specific rep type selection
            logger_1.default.info('Applying filters...');
            // Set date range - always set it since "This Year" is the default
            logger_1.default.info(`Setting date range to: ${dateRange}`);
            try {
            ...
            await download.saveAs(tempFilePath);
            // Get filenames and folder
            const filenames = this.getCustomFilenames('salesrep');
            const targetFolder = this.getFolderForDateRange(dateRange);
            // Create folder if it doesn't exist
            if (!fs.existsSync(targetFolder)) {
                fs.mkdirSync(targetFolder, { recursive: true });
