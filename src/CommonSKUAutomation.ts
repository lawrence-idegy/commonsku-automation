...
export class CommonSKUAutomation {
  ...
  private maxRetries = config.retry.maxRetries;
  private retryDelay = config.retry.retryDelay;
  private sessionFilePath = path.join(process.cwd(), 'state', 'browser-session.json');
  private googleDriveUploader: GoogleDriveUploader | null = null;

  constructor() {
    logger.info('CommonSKU Automation initialized');
    // Ensure download directory exists
    if (!fs.existsSync(config.browser.downloadPath)) {
      fs.mkdirSync(config.browser.downloadPath, { recursive: true });
      logger.info(`Created download directory: ${config.browser.downloadPath}`);
    }
    ...
    if (config.googleDrive.enabled) {
      try {
        ...
        logger.warn('Failed to initialize Google Drive uploader. Uploads will be disabled.', error);
  ...
  private async isLoggedIn(): Promise<boolean> {
    ...
    try {
      ...
      await this.page.goto(`${config.commonsku.baseUrl}/report/sales-dashboard`, {
      ...
      logger.debug(`Session check: ${loggedIn ? 'active' : 'expired'}`);
      return loggedIn;
    } catch (error) {
  ...
  private async ensureLoggedIn(): Promise<boolean> {
    ...
    if (loginSuccess) {
      logger.info('Successfully re-logged in after session expiry');
      return true;
    }

    logger.error('Failed to re-login after session expiry');
    return false;
  }
   ...
   */
  private async uploadToGoogleDrive(filePath: string | null, dateRange?: string): Promise<void> {
    if (!filePath) {
      logger.debug('No file to upload (filePath is null)');
      return;
    }

    if (!config.googleDrive.enabled) {
      logger.debug('Google Drive upload is disabled');
      return;
    }

    if (!this.googleDriveUploader) {
      logger.warn('Google Drive uploader not initialized');
      return;
    }

    if (!config.googleDrive.uploadAfterEachReport) {
      logger.debug('Upload after each report is disabled');
      return;
    }

    try {
      logger.info(`Uploading to Google Drive: ${path.basename(filePath)}`);

      const result = await this.googleDriveUploader.uploadFile(filePath, {
        organizationType: config.googleDrive.organizationType,
        transferCount: config.googleDrive.transferCount,
        showProgress: config.googleDrive.showProgress,
        dateRange: dateRange, // Pass dateRange to determine folder
      });

      if (result.success) {
        logger.info(`Successfully uploaded ${path.basename(filePath)} to Google Drive (${result.bytesTransferred} bytes in ${result.duration}ms)`);
      } else {
        logger.error(`Failed to upload ${path.basename(filePath)} to Google Drive:`, result.errors.join(', '));
      }
    } catch (error) {
      logger.error('Error during Google Drive upload:', error);
    }
  }
   ...
   */
  private getFolderForDateRange(dateRange: string): string {
    const baseDir = config.browser.downloadPath;
   ...
   */
  private getCustomFilenames(reportType: string, dateRange: string): { primary: string; secondary: string } {
    const now = new Date();
  ...
  async initialize(): Promise<void> {
    try {
      ...
      this.context.setDefaultTimeout(config.browser.actionTimeout);
      this.context.setDefaultNavigationTimeout(config.browser.navigationTimeout);
      ...
      this.page.on('download', async (download) => {
        try {
          ...
          const filePath = path.join(config.browser.downloadPath, suggestedFilename);
          await download.saveAs(filePath);
          logger.info(`Downloaded: ${suggestedFilename}`);
        } catch (error) {
  ...
  async login(): Promise<boolean> {
    ...
    while (attempts < this.maxRetries) {
      try {
        attempts++;
        logger.info(`Login attempt ${attempts} of ${this.maxRetries}...`);
        
        // Navigate to the base URL
        await this.page.goto(config.commonsku.baseUrl, {
          waitUntil: 'domcontentloaded',
          timeout: 30000,
        });
        ...
        if (!currentUrl.includes('/login') && !currentUrl.includes('/signin') && !currentUrl.includes('/auth')) {
          try {
            ...
            logger.info('Not logged in, proceeding to login page...');
          }
        }
        ...
        if (!currentUrl.includes('/login') && !currentUrl.includes('/signin')) {
          await this.page.goto(`${config.commonsku.baseUrl}/login`, {
            waitUntil: 'domcontentloaded',
            timeout: 30000,
          });
          await this.page.waitForTimeout(3000);
        ...
        for (const selector of emailSelectors) {
          try {
            ...
            if (emailField) {
              logger.info(`Found email/username field with selector: ${selector}`);
              break;
            }
          } catch (e) {
        ...
        await emailField.type(config.commonsku.username, { delay: 50 });
        ...
        await passwordField.type(config.commonsku.password, { delay: 50 });
        ...
        if (loginButton) {
          ...
          logger.info('Could not find login button, pressing Enter...');
        ...
        if (loginSuccess) {
          ...
          logger.error(`Login attempt ${attempts} failed. Still on login page.`);
        ...
        logger.error(`Login attempt ${attempts} failed:`, error);
        if (attempts < this.maxRetries) {
          logger.info(`Waiting ${this.retryDelay}ms before retry...`);
          await this.page.waitForTimeout(this.retryDelay);
        }
    ...
    logger.error(`Failed to login after ${this.maxRetries} attempts`);
    return false;
  }
  
  async exportDashboardReport(dateRange: string = 'Today'): Promise<string | null> {
    if (!this.page) throw new Error('Browser not initialized');
    ...
    try {
      logger.info('Navigating to dashboard report...');
      ...
      try {
        ...
        for (const selector of clientRepSelectors) {
          ...
          if (element) {
            ...
            logger.info('Selected Client Rep');
            break;
          }
        }
      } catch (e) {
        logger.warn('Could not select Client Rep:', e);
      }
      
      // Select Sales Team - using native Playwright clicks for React components
      try {
        logger.info('Selecting Sales Team...');
        ...
        try {
          ...
          logger.info('Clicked Select Group');
          await this.page.waitForTimeout(2000);

          // Now click "Sales Team"
          await this.page.click('text=/Sales Team/i', { timeout: 3000 });
          logger.info('Selected Sales Team ✓');
          await this.page.waitForTimeout(1000);
        } catch (e) {
          logger.warn('Could not navigate to Sales Team, will use default filter');
        }
      } catch (e) {
        logger.warn('Could not select Sales Team:', e);
      }
        ...
        if (dateInput) {
          ...
          for (const option of dateOptions) {
            ...
            if (isVisible) {
              await option.click();
              await this.page.waitForTimeout(1000);
              logger.info(`Selected date range: ${dateRange}`);
              break;
            }
          }
        } else {
          logger.warn('Date input not found');
        }
      } catch (e) {
        logger.warn(`Could not set date range to ${dateRange}:`, e);
      }
      ...
      for (const selector of getReportSelectors) {
        ...
        if (element) {
          ...
          logger.info('Clicked Get Report button');
          break;
        }
      }
      
      if (!reportGenerated) {
        logger.warn('Could not find Get Report button, continuing anyway...');
      }
      ...
      for (const selector of exportSelectors) {
        try {
          ...
          if (element) {
            await element.click();
            exportClicked = true;
            logger.info(`Clicked export with selector: ${selector}`);
            break;
          }
        } catch (e) {
          // Continue trying other selectors
      ...
      const tempFilePath = path.join(config.browser.downloadPath, tempFileName);
      ...
      if (!fs.existsSync(targetFolder)) {
        ...
        logger.info(`Created folder: ${targetFolder}`);
      }
      ...
      for (const filename of files) {
        ...
        logger.info(`Dashboard report exported: ${filename} to ${path.basename(targetFolder)}/`);
      }
      ...
      try {
        const screenshotPath = path.join(config.browser.downloadPath, `error_dashboard_${Date.now()}.png`);
  ...
  async exportPipelineReport(dateRange: string = 'Today'): Promise<string | null> {
    if (!this.page) throw new Error('Browser not initialized');
    ...
    try {
      logger.info('Navigating to pipeline report...');
      
      // Navigate to pipeline report - CORRECTED URL
      await this.page.goto(`${config.commonsku.baseUrl}/report/sales-pipeline`, {
        waitUntil: 'domcontentloaded',
        timeout: 30000,
      });
      ...
      logger.info('Selecting Order Rep type...');
      ...
      if (orderRepButton) {
        ...
        if (isSelected !== 'true') {
          ...
          await this.page.waitForTimeout(1000);
          logger.info('Selected Order Rep');
        } else {
          logger.info('Order Rep already selected');
        }
      } else {
        logger.warn('Order Rep toggle not found');
      }
      ...
      for (const filename of files) {
        ...
        // Copy the file
        fs.copyFileSync(tempFilePath, finalFilePath);
        logger.info(`Pipeline report exported: ${filename} to ${path.basename(targetFolder)}/`);
      }
      ...
      logger.error('Failed to export pipeline report', error);
      return null;
    }
  }
  
  async exportSalesOrdersReport(dateRange: string = 'This Week'): Promise<string | null> {
    if (!this.page) throw new Error('Browser not initialized');
    ...
    try {
      logger.info('Navigating to sales rep report...');
      
      // Navigate to sales rep report - CORRECTED URL
      await this.page.goto(`${config.commonsku.baseUrl}/report/sales-rep`, {
        waitUntil: 'domcontentloaded',
        timeout: 30000,
      });
      ...
      try {
        logger.info('Attempting to select Form Type: Sales Order...');
        ...
        if (formTypeContainer) {
          ...
          if (dropdown) {
            ...
            if (salesOrderOption) {
              ...
              logger.info(`Sales Order option visible: ${isVisible}`);
              ...
              if (isVisible) {
                ...
                logger.warn('Sales Order option found but not visible');
              }
            } else {
              logger.warn('Sales Order option not found in dropdown');
            }
          } else {
            logger.warn('Dropdown control not found in Form Type container');
          }
        } else {
          logger.warn('Form Type container with data-testid not found');
        }
      } catch (e) {
        logger.error('Failed to select Form Type:', e);
      }
      ...
      for (const selector of actionsSelectors) {
        try {
          ...
          if (element && await element.isVisible()) {
            await element.click();
            actionsOpened = true;
            logger.info(`Opened Actions dropdown with selector: ${selector}`);
            await this.page.waitForTimeout(1500);
            break;
          }
        } catch (e) {
      ...
      if (!actionsOpened) {
        logger.warn('Could not open Actions dropdown with standard selectors, trying alternative methods...');
        ...
        for (const btn of actionButtons) {
          ...
          if (text && text.toLowerCase().includes('action')) {
            await btn.click();
            actionsOpened = true;
            logger.info('Found and clicked Actions button via text search');
            await this.page.waitForTimeout(1500);
            break;
          }
        }
      ...
      if (!exportClicked) {
        ...
        const screenshotPath = path.join(config.browser.downloadPath, `error_sr_export_${Date.now()}.png`);
        await this.page.screenshot({ path: screenshotPath, fullPage: true });
      ...
      for (const filename of files) {
        ...
        // Copy the file
        fs.copyFileSync(tempFilePath, finalFilePath);
        logger.info(`Sales rep report exported: ${filename} to ${path.basename(targetFolder)}/`);
      }
      ...
      logger.error('Failed to export sales rep report', error);
      return null;
    }
  }
  
  async exportAllReports(): Promise<{ dashboard: string | null; pipeline: string | null; salesOrders: string | null }> {
    const results = {
    ...
    try {
      ...
      logger.info('All reports exported successfully', results);
    } catch (error) {
...
export default CommonSKUAutomation;
